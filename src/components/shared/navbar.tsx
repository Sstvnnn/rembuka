...
export function Navbar({ user }: NavbarProps) {
  ...
  const isAuthPage = pathname === "/login" || pathname === "/register" || pathname === "/verify-otp" || pathname === "/forgot-password" || pathname === "/reset-password";
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-500 pointer-events-none">
      <div className={cn(
        ...
        {/* Brand */}
        <Link href={isAdmin ? "/admin/queue" : "/home"} className="flex items-center gap-3 group">
          <div className={cn(
            "flex items-center justify-center rounded-xl bg-[#3F5C73] text-white transition-all duration-500 shadow-[#3F5C73]/20",
            isScrolled ? "size-9 shadow-lg" : "size-10 shadow-xl"
          )}>
            <ShieldCheck className={isScrolled ? "size-4" : "size-5"} />
          </div>
          <div className={cn("transition-all duration-500", isScrolled ? "hidden sm:block" : "block")}>
            <h1 className={cn("font-black text-slate-800 tracking-tight uppercase leading-none", isScrolled ? "text-xs" : "text-lg")}>Rembuka</h1>
            {!isScrolled && (
              <p className="text-[10px] font-bold text-[#4FB3B3] uppercase tracking-widest leading-none mt-1">
                {isAdmin ? "Admin Console" : "Civic Network"}
              </p>
            )}
          </div>
        </Link>
        ...
        <div className={cn(
          ...
          isScrolled ? "bg-slate-900/5 p-1 rounded-full border border-slate-900/5" : "gap-4 md:gap-8"
        )}>
          {links.map((link) => {
            ...
            return (
              <Link 
                ...
                className={cn(
                  ...
                  isScrolled 
                    ? cn("px-4 py-1.5 text-[10px] font-bold rounded-full", isActive ? "text-white" : "text-slate-500 hover:text-slate-800")
                    : cn("px-0 py-2 text-sm font-bold", isActive ? "text-[#3F5C73]" : "text-slate-400 hover:text-slate-800")
                )}
              >
                {isScrolled && isActive && (
                  <motion.div 
                ...
                <link.icon className={cn("relative z-10 transition-all duration-300", isScrolled ? "size-3.5" : "size-4", isActive && !isScrolled ? "text-[#3F5C73]" : "")} />
                <span className={cn("relative z-10", isScrolled ? "hidden md:block" : "block")}>{link.name}</span>
                {!isScrolled && isActive && (
                  <motion.div 
          ...
          <div className="relative">
            <button 
              ...
              className={cn(
                ...
                isScrolled ? "pl-2 pr-3 py-1 bg-white/80 border border-slate-100 shadow-sm" : "p-1"
              )}
            >
              <div className={cn(
              ...
              <ChevronDown className={cn("size-3 text-slate-400 transition-transform duration-300", isProfileOpen && "rotate-180")} />
            </button>
                ...
                <>
                  ...
                  >
                    <div className="px-4 py-3 border-b border-slate-50 mb-1">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1.5">User Identity</p>
                    ...
                    <form action="/signout" method="post">
                      ...
                      >
                        <LogOut className="size-4 text-rose-400" /> Secure Sign Out
                      </button>
                    </form>
                  </motion.div>
                </>
