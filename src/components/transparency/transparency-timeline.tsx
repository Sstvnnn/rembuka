    ...
    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 max-w-3xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10 pb-6 border-b border-slate-100">
        <div>
          ...
          <h2 className="text-2xl font-heading font-black text-[#1A1F2B] tracking-tight">
      ...
      <div className="relative pl-6 space-y-8 before:absolute before:inset-0 before:ml-[1.4rem] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-blue-200 before:via-slate-200 before:to-transparent">
        ...
        {logs.map((log, index) => (
          ...
          >
            {/* Ikon Lingkaran */}
            <div className="absolute left-[-1.95rem] md:left-auto md:-ml-[1.35rem] flex items-center justify-center w-10 h-10 rounded-full border-4 border-white bg-[#F6F5F2] shadow-sm z-10 group-hover:scale-110 transition-transform">
              {getIconForStatus(log.new_status)}
            </div>

            {/* Konten Log */}
            <div className="flex-1 bg-[#F6F5F2]/50 rounded-2xl p-5 border border-slate-100 group-hover:border-blue-100 group-hover:bg-blue-50/30 transition-colors ml-4 md:ml-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                <span className="font-bold text-[#1A1F2B] text-sm">
