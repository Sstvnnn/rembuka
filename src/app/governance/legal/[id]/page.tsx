...
export default function LegalStatementAdminPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (!analysisRes.ok) {
        throw new Error(analysisJson.error || "Gagal memuat legal analysis.");
      }
      ...
      setError(
        err instanceof Error ? err.message : "Gagal memuat data statement.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function saveStatement(statementId: string) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Gagal menyimpan statement.",
      );
    } finally {
      setSavingId(null);
    }
          ...
          <div className="relative z-10 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div className="space-y-3">
              ...
              <h1 className="text-4xl font-black tracking-tight text-slate-800">
                {legal?.file_name || "Kelola Statement Regulasi"}
              </h1>
              <p className="max-w-3xl text-sm font-medium leading-relaxed text-slate-500">
                Approve atau ubah statement untuk regulasi ini sebelum dipakai
                di halaman voting dan analisis.
              </p>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <ButtonLikeLink href="/legal/admin" label="Kembali" />
              <ViewPdfButton
          ...
          {statements.length === 0 ? (
            <div className="rounded-[2rem] border border-dashed border-slate-200 bg-slate-50 p-12 text-center">
              ...
              <p className="mx-auto mt-2 max-w-sm text-sm text-slate-500">
                Tambahkan statement dari proses analisis terlebih dahulu agar
                bisa di-approve di sini.
              </p>
            </div>
          ) : (
                      ...
                      <div className="min-w-0 flex-1 space-y-3">
                        ...
                        <div className="flex flex-wrap items-center gap-3 pt-2">
                          {isEditing ? (
                            ...
                            <>
                              <button
                                onClick={() => toggleApprove(statement)}
                                disabled={savingId === statement.id}
