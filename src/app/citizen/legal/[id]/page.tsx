...
export default function LegalDetailPage() {
  ...
  const [statements, setStatements] = useState<
    { id: string; text: string; [key: string]: unknown }[]
  >([]);
  ...
  useEffect(() => {
    ...
    init();
  }, [supabase, fetchStatements, fetchLegal, fetchVotes]);
        ...
        <div>
          ...
          <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="rounded-2xl bg-white border border-slate-100 shadow-sm p-5">
              <p className="text-xs font-medium text-slate-400">Pernyataan</p>
          ...
          <section className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    {legal?.file_name || "Dokumen belum tersedia"}
                  </p>
                </div>
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button
                    onClick={() => router.push(`/legal/${legalId}/analysis`)}
                  ...
                  <div className="mt-6 space-y-3">
                    ...
                    {statements.length === 0 ? (
                      <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-6 py-10 text-center text-sm text-slate-500">
                        Belum ada pernyataan yang disetujui.
                      </div>
                    ) : (
                      statements.map((s) => {
            ...
            >
              ...
              <p className="text-sm text-slate-500 leading-relaxed mb-4">
                Sampaikan masukan singkat dan langsung untuk regulasi ini.
              </p>
              ...
              <div className="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-500">
                Opini yang dikirim akan membantu proses penilaian kebijakan.
              </div>
            </motion.div>
          </section>
        </div>
